...
const Input = ({ label, error, className = '', ...props }) => {
    return (
        <div className={`flex flex-col gap-1 ${className}`}>
            ...
            <input
                className={`border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-cge-blue transition-shadow 
                bg-white dark:bg-slate-800 dark:text-white dark:border-slate-600
                ${error ? 'border-cge-red focus:ring-cge-red' : 'border-gray-300'}`}
                {...props}
            />
